...
function OrderMoreInfo() {
  ...
  const orderNumber: string | undefined = order.number;

  //Сделаем новый массив ингредиентов в заказе в котором в объектах будет кол-во повторений этого ингредиента
  const orderIngrCount: Array<{ IIngredient } | null> = allIngredients.reduce(
    (prevVal: [], item: IIngredient) => {
  ...
  const total = () => {
    return orderIngrCount.reduce(
      (acc: number, item: IIngredient) => acc + item.price * item.quantity,
      0
    );
  };
